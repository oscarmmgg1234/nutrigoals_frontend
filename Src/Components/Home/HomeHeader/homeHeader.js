...
class HomeHeader extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevState.searchModalV != this.state.searchModalV &&
      this.state.searchModalV != ''
    ) {
      this.setData([]);
      this.setSearchI('');
    }
      ...
      <user_context.Consumer>
        {(user_context) => (
          <View style={Styles.headerWrapper}>
                ...
                {(app_context) => (
                  <TouchableOpacity onPress={app_context.selectImage}>
                    <Image
                      ...
                      style={[
                        Styles.profileStyle,
                        {borderColor: 'rgba(20,20,30,0.6)', borderWidth: 4},
                      ]}
                    />
                  </TouchableOpacity>
                )}
            ...
            <CalendarComponent
              visibility={this.state.calendarModalVisible}
              removeVis={this.removeCalendarModalView}
              editDate={user_context.editDate}
              date={this.state.date}
              markedDate={user_context.markedDate}
              setMarkedDate={user_context.setMarkedDate}
            />
            <View style={Styles.headerContainer}>
              ...
              <View style={{marginLeft: -10}}>
                <TouchableOpacity
                  onPress={() => this.setState({calendarModalVisible: true})}
